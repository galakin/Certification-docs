...
The message deduplication ID is the token used for the deduplication of sent messages. If a message with a particular message 
deduplication ID is sent successfully, any messages sent with the same message deduplication ID are accepted successfully but 
...
DynamoDB Minimum requiremet for an upsert operation is GetItem and UpdateItem
...
ElastiCache is \textbf{NOT a supported destination} for Amazon Kinesis Data Firehose.\\

Kinesis data stream allow tou to write your own stream data handler. Kinesis firehose is a fully managed stream
data handler integrated with AWS resources.\\
...
the Scan operation can logically divide a table or secondary index into multiple segments, with multiple application 
workers scanning the segments in parallel with the \textbf{parallel scan} operation.\\

The \textbf{DeleteTable} operation deletes a table and all of its items. After a DeleteTable request, the specified 
table is in the \textbf{DELETING} state until DynamoDB completes the deletion.\\
\textbf{PurgeTable} DON'T exist as an operation

Amazon ElastiCache Redis on cluster mode is the best solution for RDS databases.\\ 

Use ElastiCache to mantain user session on a EC2 instance during a B/G deployment.\\

Only vertical scaling is available for ElastiCache when \textbf{cluster mode} is disabled.\\
...
Use scheduled scaling to increase provisioned concurrency in anticipation of peak traffic. To increase provisioned concurrency 
automatically as needed, use the Application Auto Scaling API to register a target and create a scaling policy.\\

AWS Lambda service does not support Lambda functions that use multi-architecture container images.\\

You need to create a deployment package if you use the Lambda API to manage functions, or if you need to include libraries 
and dependencies other than the AWS SDK.\\

If you create a stage variable to call a Lambda function through your API, you must add the required permissions.\\ 
Update your Lambda function's resource-based AWS Identity and Access Management (IAM) policy so that it grants invoke permission 
to the API Gateway.\\

By allocating provisioned concurrency before an increase in invocations, you can ensure that all requests are served by 
initialized instances with low latency.\\ 

\textbf{Stage variables} are name-value pairs that you can define as configuration attributes associated with a deployment stage of an API. 
...
you can manage multiple release stages for each API, such as alpha, beta, and production.\\
...
An io1 volume can range in size from 4 GiB to 16 TiB. The maximum ratio of provisioned IOPS to the requested volume size (in GiB) is 50:1. 
...
To save data stored in a DV deployed with ElasticBeanstalk \textbf{make a snapshot} of the DB.\\
...
You can configure multiple environment on ElasticBeanstalk.\\
...
SecurityGropu are steteful, allowing inbound traffic to one port enable the outbound connection.\\
Network ACL are stateless, you need to allow connection on both inbound and outbound traffic.\\
...
Use the \textbf{sam sync} command from the AWS Serverless Application Model (AWS SAM) to deploy incremental changes-\\
The sam sync command is specifically designed to enable rapid iteration by synchronizing local changes with the deployed 
serverless application on AWS. This allows developers to quickly test incremental changes without the overhead of 
redeploying the entire stack, aligning perfectly with the developer's requirements.\\

The \textbf{AWS Serverless Application Repository} is a managed repository for serverless applications. It enables teams, 
organizations, and individual developers to store and share reusable applications, and easily assemble and deploy serverless 
...
\textbf{'x-amz-server-side-encryption': 'aws:kms'} is the header that hould be used for encrypting file on S3.\\
...
\textbf{--projection-expression} CLI option allow the rewtrieval of a subset of attribuites froma DynamoDB scan operation.\\
...
\textbf{Origin Groups} are for origin failure scenarios and not for request routing.\\

CloudFront can comminicate via HTTPS with both client and backend.\\

Cloudfront origins are: S3 bucket and custom HTTP origin.\\
...
To export a stack's output value, use the \textbf{Export} field in the Output section of the stack's template. 
To import those values, use the Fn::ImportValue.\\

You can referene code writteon on inline editor for CloudFormation as long as there are no other depnedency
...
AWS CloudFormation \textbf{Condition} resources can't be associated to \textbf{Parameter} resources
...
VPC Flow Logs - VPC Flow Logs is a feature that enables you to capture information about the IP traffic going 
to and from network interfaces in your VPC.\\
You can create a flow log for a VPC, a subnet, or a network interface. If you create a flow log for a subnet or 
VPC, each network interface in that subnet or VPC is monitored.\\
...
The Classic Load Balancer doesn't allow you to run multiple copies of a task on the same instance. Instead, with the Classic Load Balancer, 
you must statically map port numbers on a container instance.\\

General Purpose SSD (gp2) volumes offer cost-effective storage that is ideal for a broad range of workloads. These volumes deliver single-digit millisecond 
latencies and the ability to burst to 3,000 IOPS for extended periods of time. Between a minimum of 100 IOPS (at 33.33 GiB and below) and a maximum of 
16,000 IOPS (at 5,334 GiB and above), baseline performance scales linearly at 3 IOPS per GiB of volume size.\\

EC2 support B/G and In-place deployment.\\

\section{Route 53}
CNAME records can be used to map one domain name to another. Although you should keep in mind that the DNS 
protocol does not allow you to create a CNAME record for the top node of a DNS namespace.\\
...
\begin{itemize}
	...
	\item Cognito User pool
 	\item Lambda Authorizer
\end{itemize}
AWS Api Gateway can not use AWS STS as an authorization method.\\
...
The client must send a request that contains the \textbf{"Cache-Control: max-age=0" header}.\\ 

API Gateway lets you use \textbf{mapping templates} to map the payload from a method request to the corresponding integration request and 
from an integration response to the corresponding method response.\\

A \textbf{usage plan} specifies who can access one or more deployed API stages and methods—and also how much and how fast they can access them.\\ 
You can configure usage plans and API keys to allow customers to access selected APIs at agreed-upon request rates and quotas 
that meet their business requirements and budget constraints.\\
...
CORS allow sharing file hosted on S3 with another origin (S3 bucket).\\
...
Using server-side encryption with customer-provided encryption keys (\textbf{SSE-C}) allows you to set your encryption keys.\\

AWS s3 versioning is available only on \textbf{bucket level}.\\

AWS S3 use the \textbf{AES256} encrypt algorithm  to upload file over S3, this is the value that need to be 
specified con the buckete policy to ensure the upload of an encrypted file
...
To calculate the WCU require use the following formula:\\
\[
  WCU = \frac{item}{second}*\frac{size KB}{1 KB}
\]
WCU is 2$*$WCU if there is a trasactional write\\

To calculate the RCU require use the following formula:\\
\[
  RCU = \frac{item}{2 * second}*\frac{size KB}{4 KB}
\]
...
IAM \textbf{Access Analyzer} simplifies inspecting unused access to guide you toward least privilege. 
Security teams can use IAM Access Analyzer to gain visibility into unused access across their AWS organization and automate 
how they rightsize permissions.\\

\textbf{Trust policy} is the only resource bound policy supported by AWS IAM.\\

\section{AWS CodeBuildandd CodeCommit}
...
IAM Username and Password \textbf{are not supported} to access AWS CodeCommit.\\

AWS CodeDeploy action order is the following:\\
ApplicationStop, DownloadBundle, BeforeInstall, ApplicationStart, ValidateService.\\

The simplest way to set up connections to AWS CodeCommit repositories is to configure Git credentials for CodeCommit in the IAM 
console, and then use those credentials for HTTPS connections.
...
architecture. With X-Ray, you can understand how your application and its underlying services are performing to identify and 
troubleshoot the root cause of performance issues and errors. X-Ray provides an end-to-end view of requests as they travel 
through your application, and shows a map of your application’s underlying components.\\

You can use X-Ray to collect data across AWS Accounts.\\

If you can't see traced on an X-Ray Enabled AWS resources it mean that the IAM Role associated with the resources is not
configured so enable the metrics exposure through AWS X-Ray
...
With fargate ECS deployment you can't use daemon as you don't have access to the instances of the ECS cluster.\\
\section{Cognito}
...
Use \textbf{associte-kms-key} CLI command to encrypt a CloudWatch log group.\\

EventBridge is used to integrate Cloudwatch to trigger event on different AWS resoruces.\\
...
\section{AWS LoadBalancer}
Elastic Load Balancer is a managed load balancer.\\
Application load balancers is Layer 7 (HTTP)\\
Load balancing to multiple HTTP applications across machines (target groups)\\  
Load balancing to multiple applications on the same machine (ex: containers)\\
ALB are a great fit for micro services and container-based application\\
ALB taget are: EC2 instances, ECS task, Lambda functions of private IPs.\\

You can not specify public routable IP address to an ALB target

NLB has one static IP for AZs and support assigning Elastic IPs.
NLB are use for extreme performance with TCP and UDP protocols

NLB target are: EC2 instances, private IPs and ALBs\\

\textbf{Connection Draining}:  Time to complete “in-flight requests” while the instance is de-registering or unhealthy.\\
...
\textbf{ALB access logs} - Elastic Load Balancing provides access logs that capture detailed information about 
...
The percentage of traffic received through a ELB is equally divided between instances in the same Region 
\textbf{regarding of the Available Zone}.
