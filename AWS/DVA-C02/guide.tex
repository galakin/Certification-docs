...
ynamoDB Streams enables powerful solutions such as data replication within and across AWS Regions, materialized 
...
ambda usage that is covered by your hourly savings plan commitment is billed at the applicable CSP rate and discount. 
...
ùthat your group never goes above this size. If you specify the desired capacity, either when you create the group 
...
\subsub??section{Encrypt EBS resources}
...
\section{AWS SAM}
AWS SAM applicat can be redeployed with the \textbf{build} and \textbf{deploy} command\\
...
Routing tabkle defines the internet access for a VPN Subnet\\
...
tge EB template and it wull be deleted after redeploying the template\\
...
(IAM) policies, bucket policies, Access Control Lists (ACLs), and Query String Authentication.\\

\section{AWS RDS}
One ore more read replicas on RDS could help on read-heavy operation.\\

AWS RDS MySQL and PostGreSQL allot authentication with IAM Database Authentication
